...
```{r}
hydropower <- inflow_data %>% 
  select(my_date,HP1)

ts_hydropower <- ts(hydropower, start = c(1931,1), frequency = 12)

nobs = nrow(hydropower)
...
linear_trend_model=lm(HP1~t, hydropower)
...
ts_linear_trend <- ts(linear_trend, start=c(1931,1), frequency=12)
detrend_inflow <- hydropower$HP1 - linear_trend
ts_detrend <- ts(detrend_inflow, start=c(1931,1),frequency=12)

#visualize the trend on observed data
autoplot(ts_detrend, color="darkgreen") +
autolayer(ts_hydropower) +
autolayer(ts_linear_trend) +
ylab("HP1") +
xlab("") +
labs(color="")
```
